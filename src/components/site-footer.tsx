...
export function SiteFooter({ settings }: { settings: SiteSettings }) {
  return <footer className="site-footer">
    <div className="container">
      ...
      <nav className="footer-services" aria-label="Halaman layanan"><span className="mono">LAYANAN</span>{services.map(service => <Link key={service.slug} href={`/layanan/${service.slug}`}>{service.title}<ArrowUpRight size={13} /></Link>)}</nav>
