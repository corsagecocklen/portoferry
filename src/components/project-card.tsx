...
export function ProjectCard({ project, index = 0 }: { project: Project; index?: number }) {
  ...
  return <article className="project-card">
    <div className="post-header"><Image className="post-avatar" src="/images/ferry-avatar.webp" alt="" width={30} height={30} /><div><span className="post-author">ferry.kurniawan</span><span className="post-subtitle">{project.category}</span></div><span className="post-index mono">0{index + 1}</span><button className="icon-button post-more" onClick={share} aria-label={`Salin tautan ${project.title}`}><MoreHorizontal size={19} /></button></div>
    <Link href={`/proyek/${project.slug}`} className="project-art" aria-label={`Lihat proyek ${project.title}`}>
